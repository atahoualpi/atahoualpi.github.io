...
function fillRestaurantsSVG(svgID){
...
//var X = -122;
//var Y = 37;
var color_list = ['yellow','limegreen','red','brown','magenta','blue','cyan'];
...
var restaurantSymbolMask = d3.select("defs").append("svg:mask").attr("id","restaurantSymbolMask");
restaurantSymbolMask.append("circle").attr("cx",0).attr("cy",0).attr("r",50).attr("fill","white");
...
var svg = d3.select('#'+svgID)//.select("#"+"map_canvas")//.select("body")
    ...
    var myTool = d3.select("#myTooltip");/*d3.select("defs")
        .append("div")
        .attr("class", "tooltip")
        .html("CIAO GIANNI");*/
...
var restaurants, restaurantsCenters;
    //acquire and display restaurants
    d3.csv("restaurants_data.csv", function (data) {

        restaurants = svg.selectAll("restaurants")
            .data(data)
            .enter()
            .append("circle")
            .attr("d", path)
            ...
            .attr('street', function(d){return d['Street address']})
            .attr('seats', function(d){return d['Number of seats']})
            .style("stroke-width", "0.5")
            .style("stroke", "black")
            .on("mouseover", function(d) {  //Mouse event

                restaurants.filter(function(){ return d3.select(this).attr('cluster') == d.Cluster; })
            ...
            .on("click",function(){
                ...
                var ownCenter = restaurantsCenters.each(function(){
    ...
    //acquire and display Cluster Centers of restaurants
        var myRe = /\+*\-*[0-9]+\.[0-9]+/g; //defined here but it will be used later
    d3.csv("restaurants_clusters.csv", function (data) {

        var clusterIndex =0;
       restaurantsCenters= svg.selectAll("centers")
            .data(data)
            .enter()
           .append("g")
           .attr("cluster", function(d,i){clusterIndex=i; return i;})
           /*.call(function(group){
        group.attr("mask","url(#restaurantSymbolMask)");//.attr("transform","translate(300,300)scale(0.5)");
               console.log("here");
           ...
           // .style("stroke", "black")
            .on("mouseover", function(d,i) {  //Mouse event
                ...
                restaurants.filter(function(){ return d3.select(this).attr('cluster') == i; })
            ...
            .on("click", function(){
                ...
                defaultInteractionFunctions.push([]);//'restaurants');
                defaultInteractionFunctions[0].push(restaurants.on("mouseover"));restaurants.on("mouseover",null);
                defaultInteractionFunctions[0].push(restaurants.on("mouseout"));restaurants.on("mouseout",null);
                defaultInteractionFunctions[0].push(restaurants.on("click"));restaurants.on("click",null);
                defaultInteractionFunctions.push([]);//'restaurantsCenters');
                defaultInteractionFunctions[1].push(restaurantsCenters.on("mouseover"));restaurantsCenters.on("mouseover",null);
                defaultInteractionFunctions[1].push(restaurantsCenters.on("mouseout"));restaurantsCenters.on("mouseout",null);
                defaultInteractionFunctions[1].push(restaurantsCenters.on("click"));restaurantsCenters.on("click",null);
                restaurants.filter(function(){ return d3.select(this).attr('cluster') == i; })
                    .each( function(){
                        ...
                        d3.select(this)
                            .on("mouseover", function(){  //Mouse event
                                ...
                                //var window_offset = $('#restaurantsSVG').offset().top - $(window).scrollTop();
                                ...
                                myTool.html("<h5><b>" +
                                    ...
                                    "<p>n° of seats: " + Math.floor(d3.select(this).attr('seats'))+"</p>" +
                                    "<p>Geo Position: (" +lon+','+lat+")</p>" +
                                    "</b></h5>");
                                tooltipDimensions = [myTool.node().offsetWidth,myTool.node().offsetHeight];
                                var tooltipParentOffset = $('#myTooltip').parent().offset();
                                var svgOffset =     $('#restaurantsSVG').offset();
                                ...
                                /* .attr("transform", function(d, i) {
                                 return "translate(" + pixelPos + ")";
                                    ...
                                    .style("top",  ( svgOffset.top - tooltipParentOffset.top + pixelPosInSVG[1] - tooltipDimensions[1] - 10)+"px")//window.pageXOffset + matrix.f- tooltipDimensions[1] - 10
                ...
                ;
                restaurantsCenters.filter(function(){ return d3.select(this).attr('cluster') == i; })
                    .each( function() {
                        d3.select(this)
                            .on("mouseover", function(){  //Mouse event
                                ...
                                myTool
                                    ...
                                    .style("top",  ( svgOffset.top - tooltipParentOffset.top + pixelPosInSVG[1] - tooltipDimensions[1] - 10)+"px")
                                    .style("border", function(){
                                        return "solid "+color_list[i]; //HIGH Danger
                                    })
                                    .style("opacity", "1")
                ...
                //restaurants.each(function(){console.log(1);});


                //for each restaurants of the clicked cluster, gather - longitude and latitude
                var latValues = []; var lonValues = [];
                filtered_restaurants = restaurants.filter(function(){ return +d3.select(this).attr('cluster') == i; });
                filtered_restaurants.each(function(){
               ...
               // console.log(lonMax,lonMin,latMax,latMin);
                ...
                restaurants.style('opacity',0);
                restaurantsCenters.style('opacity',0);
...
//appearance of the points restaurants while zoomed
                restaurants.filter(function(){ return d3.select(this).attr('cluster') == i; })
                    .style("fill", color_list[i]).attr("r",pointRadiusOnZoom).style("stroke-width", pointStrokeWidthOnZoom);
                restaurantsCenters.filter(function(){ return d3.select(this).attr('cluster') != i; }).selectAll("rect").style("fill", "grey");/*.each(function(){
                    //console.log(d3.select(this).attr("lon"));
                    d3.select(this).select("rect").style("fill", "grey");
                })*/

                restaurants.transition().duration(appearanceAnimationTime).delay(zoomAnimationTime).style('opacity',1);
                restaurantsCenters.transition().duration(appearanceAnimationTime).delay(zoomAnimationTime).style('opacity',1);
                ...
                svg.selectAll('path').on("click", function(){ //regionsPath
                    ...
                    restaurantsCenters.selectAll("rect")//.filter(function(){ return d3.select(this).attr('cluster') != i; })
                    ...
                    restaurants.on("mouseover", defaultInteractionFunctions[0][0]);
                    restaurants.on("mouseout", defaultInteractionFunctions[0][1]);
                    restaurants.on("click", defaultInteractionFunctions[0][2]);
                    restaurantsCenters.on("mouseover", defaultInteractionFunctions[1][0]);
                    restaurantsCenters.on("mouseout", defaultInteractionFunctions[1][1]);
                    restaurantsCenters.on("click", defaultInteractionFunctions[1][2]);
        ...
        restaurantsCenters.each(function(d,i){
            var group = d3.select(this);
            group.attr("mask","url(#restaurantSymbolMask)");//.attr("transform","translate(300,300)scale(0.5)");

            group.append("rect").attr("width",100).attr("height",100).attr("x",-50).attr("y",-50).style("fill",color_list[i]);
            group.append("image").attr("xlink:href","restaurantIcon.png").attr("width",70).attr("height",70).attr("transform","translate(-35,-35)");
